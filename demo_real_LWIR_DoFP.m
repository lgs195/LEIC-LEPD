...
path = './dataset/LWIR/';  % lwir
save_path = './results/LWIR/';
files = dir([path, '*.tif']);
len = length(files);
name = {};
...
for ii = 1 : 15
    ...
    [S0_leic,S1_leic,S2_leic] = polar_calibration(I0_leic,I45_leic,I90_leic,I135_leic);
    DOLP_leic = (sqrt(S1_leic.*S1_leic + S2_leic.*S2_leic))./S0_leic;
    DOLP_leic(isnan(DOLP_leic)) = 0;

    AOLP_leic = (1/2) * atan2(S2_leic,S1_leic)*180/pi;

    %% 5. visualization
    % window size
    figure('Position', [100, 100, 900, 500]);
    % show
    subplot(1, 2, 1),imshow(DOLP_leic(17:end-16,17:end-16), [0, 0.04]);colormap Parula;colorbar;title('DoLP_{color}')
    subplot(1, 2, 2),imshow(AOLP_leic(17:end-16,17:end-16), []);colormap Parula;colorbar;title('AoLP_{color}')
